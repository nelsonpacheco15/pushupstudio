...
import { REQUEST_TYPES, REQUEST_TEMPLATES } from "@/lib/tickets";
...
export default function RequestForm({ clientId, portalToken }: { clientId: string; portalToken: string }) {
  ...
  return (
    <form ref={ref} action={async (fd) => { await createTicket(fd); ref.current?.reset(); setType(REQUEST_TYPES[0]); }}
      ...
      <select name="type" value={type} onChange={(e) => setType(e.target.value)} style={{ ...inp, marginTop: 8 }}>
        {REQUEST_TYPES.map((r) => <option key={r} value={r}>{r}</option>)}
      </select>
      {template.map((f) => (
        <input key={f.name} name={`tf_${f.name}`} placeholder={f.label + (f.placeholder ? ` — ${f.placeholder}` : "")} style={{ ...inp, marginTop: 8 }} />
      ))}
      <textarea name="description" placeholder="Anything else — goal, audience, details" style={{ ...taStyle, marginTop: 8 }} rows={3} />
