...
import { STATUS_LABELS, driveEmbed } from "@/lib/tickets";
...
const T = {
  en: { back: "← Your board", brief: "BRIEF", design: "YOUR DESIGN", waiting: "Your design isn't ready yet — we'll email you the moment it is.",
    reviewed: "Your feedback", score: "score" },
  pt: { back: "← O teu quadro", brief: "BRIEFING", design: "O TEU DESIGN", waiting: "O teu design ainda não está pronto — enviamos-te um email assim que estiver.",
    reviewed: "O teu feedback", score: "nota" },
};
  ...
  return (
    <div style={{ minHeight: "100vh", background: DS.bg, color: DS.text }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "18px 26px", borderBottom: `1px solid ${DS.border}` }}>
        ...
        <Link href={`/portal/${token}`} style={{ fontFamily: DS.mono, fontSize: 12, color: DS.mute }}>{t.back}</Link>
      </div>
        ...
        {embed ? (
          <>
            ...
            {feedback.length > 0 ? (
              ...
              <TicketEvaluation token={token} ticketId={ticketId} lang={client.language} />
            )}
          </>
        ) : (
